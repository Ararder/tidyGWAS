...
The two mandatory argument is `tbl` and `dbsnp_path`. tidyGWAS accepts either a
in-memory `base::data.frame()` or a filepath to text file. If you pass a
filepath to a text file, tidyGWAS will attempt to read it using
...
if `dbsnp_path` is not passed, tidyGWAS will skip any functions relating
to dbSNP, such as repairing CHR or POS, or detecting and providing
coordinate on both genome builds.
...
`data.frame()` to contain the correct column names. `tidyGWAS_columns`
is a helper function created to match column name to the tidyGWAS
format.
