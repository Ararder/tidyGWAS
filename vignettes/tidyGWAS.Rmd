...
start <- Sys.time()
tmp <- tidyGWAS(test_file, use_dbsnp = FALSE)
end <- Sys.time()
```
...
tidyGWAS(test_file,logfile = TRUE, outdir = my_outdir, use_dbsnp = FALSE)
...
```{r, echo = FALSE, eval=FALSE}
tidyGWAS(test_file, outdir = my_outdir, name = "first_gwas", use_dbsnp = FALSE)
list.files(my_outdir, recursive = TRUE)
