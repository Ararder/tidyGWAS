...
They can be downloaded [here](https://zenodo.org).

The file needs to untarred before it can be used.

```{bash, eval = FALSE}
# zenodo link
wget https://zenodo.org/records/11060095/files/dbSNP155.tar
...
create the output folder. The folder cannot already exists - otherwise
tidyGWAS would rewrite the directory. It is recommended that have a
folder that stores all munged summary statistics. The general pattern of
`output_dir` will then be:

/file/path/to/all/cleaned/\<trait_name\>

where trait_name can be changed depending on input summary statistics.
...
We create a temporary file to store the example sumstats, and change up
some of the column names to simulate what would typically happen.
...
names. By default, tidyGWAS produces hivestyle parquet files, but if you
...
inside the folder provided by `output_dir`, you will have the following
files:
