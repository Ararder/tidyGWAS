...
The GWAS summary statistics file can be provided in-memory as a data.frame/tibble or a filepath that can be read with data.table::fread. We will use the 100k random rows from the PGC wave 3 schizophrenia [GWAS](https://pgc.unc.edu/for-researchers/download-results/) as an example file.

Here, the column names have already been renamed to the correct format.

```{r}
example_file <- dplyr::tibble(tidyGWAS::test_file)
example_file
```
...
tidyGWAS(correct_names, outdir = my_outdir, logfile = TRUE, dbsnp_path = path_to_dbsnp)
```

# Computational cost

Memory use and time scales with the size of the summary statistics. From running tidyGWAS extensively, here's an estimation of time and memory:

1.  1 - 10 mil rows: \<16gb memory, \~20 min
2.  
...
## A fast function for finding all entries in a duplication
