...
# rs_merge_arch="YOUR_FILE_PATH"
# restart R and check that this works:
data.table::fread(Sys.getenv("rs_merge_arch"), nrows = 100)
```

tidyGWAS will run all analysis in a temporary directory, and the output will not be
automatically saved. To save the output, use `outdir`.
```{r, eval = FALSE}
bs <- get_bsgenome()
rs_merge_arch <- data.table::fread(Sys.getenv("rs_merge_arch"))
sumstats_path <- "my_sumstats_filepath"
df <- data.table::fread(sumstast_path)
outdir <- getwd()
tidyGWAS(tbl = df, name = "first_tidyGWAS", outdir = outdir)

```

It's often useful to save output to a logfile
```{r, eval =FALSE}
tidyGWAS(tbl = df, logfile=TRUE, bsgenome_objects = bs, name = "first_tidyGWAS", outdir = outdir)
```
...
We can turn of the use of dbSNP, removing the majority of the running time
```{r, collapse=TRUE}
start <- Sys.time()
tmp <- tidyGWAS(test_file, rs_merge_arch = rs_merge_arch, use_dbsnp = FALSE)
end <- Sys.time()
```
...
tidyGWAS(test_file, rs_merge_arch = rs_merge_arch, logfile = TRUE, outdir = my_outdir, use_dbsnp = FALSE)
...
```{r, echo = FALSE}
tidyGWAS(test_file, rs_merge_arch = rs_merge_arch, outdir = my_outdir, name = "first_gwas", use_dbsnp = FALSE)
list.files(my_outdir, recursive = TRUE)
